...
class Prism_SubstancePainter_tester_Integration(object):
    def __init__(self, core, plugin):
        ...
        if platform.system() == "Windows":
            self.examplePath = (
                self.core.getWindowsDocumentsPath() + "\\SubstancePainter_tester\\2019"
            )
            ...
            self.examplePath = os.path.join("/home", userName, "SubstancePainter_tester", "2019")
            ...
            self.examplePath = (
                "/Users/%s/Library/Preferences/Autodesk/SubstancePainter_tester/2019" % userName
            )
    ...
    def getExecutable(self):
        execPath = ""
        if platform.system() == "Windows":
            defaultpath = os.path.join(self.getSubstancePainter_testerPath(), "SubstancePainter_tester.exe")
    ...
    @err_catcher(name=__name__)
    def getSubstancePainter_testerPath(self):
        # get executable path
        return ""
    ...
    def addIntegration(self, installPath):
        try:
            ...
            msgStr = (
                "Errors occurred during the installation of the SubstancePainter_tester integration.\nThe installation is possibly incomplete.\n\n%s\n%s\n%s"
    ...
    def removeIntegration(self, installPath):
        try:
            ...
            msgStr = (
                "Errors occurred during the removal of the SubstancePainter_tester integration.\n\n%s\n%s\n%s"
                % (str(e), exc_type, exc_tb.tb_lineno)
            )
            msgStr += "\n\nRunning this application as administrator could solve this problem eventually."
    ...
    def installerExecute(self, pluginItem, result):
        try:
            ...
            for i in pluginPaths:
                result[
                    "SubstancePainter_tester integration"
                ] = self.core.integration.addIntegration(
                    self.plugin.pluginName, path=i, quiet=True
                )
                if result["SubstancePainter_tester integration"]:
