...
# Set the path to the output file in the temp directory/temporary_master_light.usda
output_path = os.path.join(QStandardPaths.writableLocation(QStandardPaths.TempLocation), "temporary_master_light.usda")
...
core.products.createProduct(fnameData, "MasterLight_Publish", "global")

# Ingest the new version into prism
result = core.products.ingestProductVersion([output_path], fnameData,"MasterLight_Publish")
...
sticky_note.setPosition(hou.Vector2(3, -4))
sticky_note_text = "Cette scene sert a créer les lumières a la séquence.\n"
sticky_note_text += "Par défaut, tu as un node \"Import\" qui importe le set dress de la séquence.\n"
sticky_note_text += "Rends toi dans le subnet \"MASTER_LIGHT\" pour placer tes lumières.\n"
sticky_note_text += "Une fois finis, utilise le node \"USD_OUTPUT\" pour Export ta scène.\n\n"
