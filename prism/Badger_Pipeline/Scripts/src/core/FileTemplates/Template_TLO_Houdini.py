...
class FileTemplateTLOHoudini(FileTemplateBase):

    def __init__(self):
        super().__init__()
        self.template_name = "TLOHoudini"
    ...
    def construct(self, parent, path, origin):
        ...
        # Ici, on vas recuperer le light du shot courant.
        importReference_light = True
        light_Files = self.getMatchingProductsFromEntity(current_entity, [".usd", ".usda" , ".usdc"], origin, ["Light", "Publish"])
        ...
        dialog.setHelpLink("https://thomasescalle.github.io/Pipeline_USD_2025/departements/RLO/#comment-creer-une-scene-dans-houdini")
        ...
        items_light = items.get("Light", [])

        # On recupere les products associées aux items
        products_light = self.getPreferedFilePathsFromProductList(items_light, origin)
        # Si il y'a plus d'un light, on prend que le premier.
        if len(products_light) > 1 :
            products_light = [products_light[0]]


        products_light_str = str(products_light[0]) if products_light else ""
        ...
        script = StandaloneScriptHoudini("Stdl_TLO_Houdini.py", parent)
